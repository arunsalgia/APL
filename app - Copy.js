...
function updateSchedule(value) {
    if (value > 0) {
      //console.log("VAL > 0");
      if (arunSchedule === 0) {
        console.log("Next Sched started");
        arunSchedule = SECONDS5;      
        var currTime = new Date();
        currTime.setSeconds(currTime.getSeconds()+arunSchedule);
        schedJob.reschedule(currTime);
      }
    } else {
      //console.log("VAL is 0");
      if (arunSchedule > 0) {
        console.log("STopping reschedule");
        arunSchedule = 0;
        schedJob.cancel();
      }
    }
...
currencyRouter = require('./routes/currency');
