...
router.get('/timer/:value', async function (req, res, next) {
  setHeader(res);
  
  var {value} = req.params;
  aruncron = Number(value);
  
  sendok(res, aruncron.toString());
});


router.get('/matchscore/:id', async function (req, res, next) {
  setHeader(res);
  ...
  var myData = await cricapi_get_score(id);
  console.log(myData.matchType.toUpperCase());
...
router.get('/maxplayers', async function (req, res, next) {
  setHeader(res);


	let allGroups = await IPLGroup.find({});
	for(var i=0; i<allGroups.length; ++i) {
		allGroups[i]["maxPlayers"] = 15;
		allGroups[i].save();
	}
...
router.get('/playerlist', async function (req, res, next) {
	...
	try {
		...
		for(let i=0; i <myPlayers.length; ++i) {
			let tmp = {key: allPlayers[myPlayers[i]].player.key, name: allPlayers[myPlayers[i]].player.name, }
			console.log(tmp);
		}
